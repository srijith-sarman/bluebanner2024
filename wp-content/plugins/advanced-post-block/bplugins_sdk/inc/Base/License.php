...
class BSDKLicense extends BSDK{
				...
				<div class="overlay"></div>
					<div class="license_form">
						<div class="popup_header">
							<h2><?php esc_html_e("Active License", "bsdk") ?></h2>
							<span class="closer">&times;</span>
						</div>
						<div class="popup_body">
							<p><?php esc_html_e('Please enter the license key that you received in the email right after the purchase:', 'bsdk') ?></p>
							...
							<div class="terms">
								...
								<label for="agreeField"><?php echo $this->isPipe ? esc_html_e("Remove Data", 'bsdk') : sprintf( "I agreed to send the website url, email, and the License key to '%s' plugin server to verify the license key.", esc_html( $this->plugin_name ) ); ?></label>
							</div>
						</div>
						<div class="popup_footer">
                            <span class="bpl_loader"></span>
							<?php if($this->isPipe): ?>
								<input type="submit" class="button button-primary btn-deactivate" value="<?php esc_html_e("Deactive License", "bsdk") ?>" />
							<?php else: ?>
								<input type="submit" disabled="true" class="button button-primary btn-activate" value="<?php esc_html_e("Active License", "bsdk") ?>" />
							<?php endif; ?>
						</div>
					</div>
				</div>
    ...
    function syncLicense(){
        ...
        $website = sanitize_text_field( $_POST['website'] ) ?? $_SERVER['HTTP_HOST'];

...
class APBBlock extends APBPlugin{
	...
	function render( $attributes ) {
		...
		ob_start(); ?>
		<script id='firstPagePostsScript-<?php echo esc_attr( $cId ); ?>'>
			window['firstPagePosts-<?php echo esc_html( $cId ); ?>'] = <?php echo wp_json_encode( $this->getPosts( $attributes ) ); ?>;
		</script>
		<div
	...
	function loadingPlaceholder( $attributes ){
		...
		$sliderStyles = "#apbLoadingPlaceholder-$cId .apbSliderSkeleton article{
			min-height: $sliderHeight;
		}";

		ob_start(); ?>
			<div class='apbLoadingPlaceholder' id='apbLoadingPlaceholder-<?php echo esc_attr( $cId ); ?>'>
				<?php switch ( $layout ) {
					case 'grid1': ?>
